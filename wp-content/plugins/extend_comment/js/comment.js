...
var exit_e_modal_comment = e_modal_comment_form.getElementsByClassName("modal-comment");
if(exit_e_modal_comment){
  ...
  e_btn_submit.addEventListener("click",function(){
    ...
    if(!_fullname||!_phone) {
    	alert("Bạn chưa nhập họ tên hoặc số điện thoại");
    	return false;
    }
...
var e_close_modals = document.getElementsByClassName("close");
for (var i = 0; i < e_close_modals.length; i++) {
  e_close_modals[i].addEventListener("click",function(){
    var e_parent_content = this.parentElement.parentElement.parentElement;
    e_parent_content.style.display = "none";
  });
}
      ...
      for(var t = 0; t < e_item.length; t++){
        ...
        if(e_chk_true.checked === true){
  		...
  		for(var t = 0; t < e_item.length; t++){
  			...
  			if(_exist_class.classList.contains('compact-star')){
  				_exist_class.className = _exist_class.className.replace(/\bcompact-star\b/g, "");
  				_exist_class.classList.add("empty-star");
  			}
  		}
      if(max > 0){
          ...
          if(_exist_class.classList.contains('empty-star')){
            _exist_class.className = _exist_class.className.replace(/\bempty-star\b/g, "");
            _exist_class.classList.add("compact-star");
          }
        }
      }
	});  
...
//repply
var e_div_comment_repply = document.getElementsByClassName("modal-repply")[0];
var e_modal_comment_repply = e_div_comment_repply.getElementsByClassName("modal-comment")[0];
var e_comment_repply = document.getElementsByClassName("comment-reply-link");
for (var i = 0; i < e_comment_repply.length; i++) {
  e_comment_repply[i].addEventListener("click",function(){
    //console.log(this);
      var obj_ck = getCookie('ckuser');
      if(!obj_ck){
        e_modal_comment.style.display = "block";
      }else{
        e_modal_comment_repply.style.display = "block";
        //data-postid
        var commentid = this.getAttribute("data-commentid");
        var e_comment_parent = e_modal_comment_repply.getElementsByClassName("comment_parent")[0];
        e_comment_parent.value = commentid;
      }
  });
}
// var e_reply_link = document.getElementsByClassName("comment-reply-link");
